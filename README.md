...
- **必須**（実データ）: URL に **`?repo=owner/name`**。GitHub の [contributors 統計](https://docs.github.com/en/rest/metrics/statistics)の **`total`（コミット数）** を使います。  
- **任意**: **`&user=login`** で、そのリポジトリにおける **特定ユーザーのコミット数**のみ。  
- `?user` **だけ** の指定ではコミット数は取得できず、案内メッセージと **コミット 0 相当**の小さな大地になります。  
